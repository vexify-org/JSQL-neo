...
function tokenize(sql) {
  ...
  while (i < n) {
    ...
    if (c === '?') {
      ...
      tokens.push(new SQLToken('param', num ? parseInt(num, 10) : null, i));
      i = j;
      continue;
    }
...
class Parser {
  ...
  parseSelect() {
    ...
    while (true) {
      ...
      if (t.type === 'keyword' && t.value === 'COUNT') {
        ...
        columns.push({ expr: col, aggregate: 'COUNT', column: col, alias: aggregate.alias, over: aggregate.over || null, window: !!aggregate.over });
        ...
        aggregate = this.parseOptionalOver(aggregate);
        aggregate.alias = this.parseOptionalAlias();
        columns.push({ expr: col, aggregate: fn, column: col, alias: aggregate.alias, over: aggregate.over || null, window: !!aggregate.over });
    ...
    let rollup = false;
    if (groupBy && this.isKeyword('WITH')) {
      this.next();
      if (this.isWord('ROLLUP')) { this.next(); rollup = true; }
      else throw new Error(`Expected ROLLUP after WITH, got '${this.peek().value}'`);
    }
    ...
    if (this.isWord('FETCH')) {
      ...
      const nTok = this.next();
      const n = nTok.type === 'number' ? nTok.value : parseInt(String(nTok.value), 10);
  ...
  parseFromItem() {
    ...
    const NOT_ALIAS = ['JOIN', 'LEFT', 'RIGHT', 'FULL', 'WHERE', 'GROUP', 'ORDER', 'LIMIT',
      'ON', 'HAVING', 'UNION', 'INNER', 'CROSS', 'OUTER', 'INTERSECT', 'EXCEPT',
      'FETCH', 'RETURNING', 'WINDOW'];
    if (this.isKeyword('AS')) { this.next(); alias = this.parseAlias(); }
    else if (this.peek().type === 'ident' && !NOT_ALIAS.some(w => this.isWord(w))) {
  ...
  parseOperand() {
    ...
    if (t.type === 'keyword' && ['SUM', 'AVG', 'MIN', 'MAX', 'COUNT'].includes(t.value)) {
      ...
      this.expect('op', ')');
      return this.parseOptionalOver({ type: 'aggregate', fn, column });
    }
  ...
  parseComparison() {
    ...
    if ((t2.type === 'keyword' && t2.value === 'LIKE') || this.isWord('ILIKE')) {
      const isILike = !(t2.type === 'keyword' && t2.value === 'LIKE');
      this.next();
      const pattern = this.parseValue();
      return { type: 'like', operand: left, pattern, not, ci: isILike };
    }
...
function likeMatch(value, pattern, ci = true) {
  if (typeof value !== 'string') return false;
  const regex = pattern
    .replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
    .replace(/%/g, '.*')
    .replace(/_/g, '.');
  return new RegExp('^' + regex + '$', ci ? 'i' : '').test(value);
}
...
function evaluateExpr(expr, row, ctx) {
  ...
  switch (expr.type) {
    ...
    case 'compare': {
      ...
      if (expr.op === '=') return nl === nr || String(nl) === String(nr);
    ...
    case 'like': {
      const v = resolveOperand(expr.operand, row, ctx);
      const r = likeMatch(v, expr.pattern, !!expr.ci);
      return expr.not ? !r : r;
    }
    case 'quantified':
...
class SQLExecutor {
    ...
    switch (statement.type) {
          ...
          if (explicit.length > 0) {
            ...
            if (conflicts.length > 0 && conflictAction === 'replace') {
              ...
              for (const { d, existingId } of conflicts) {
                const data = {};
                if (conflictSets) {
                  for (const [col, val] of conflictSets) data[col] = val;
                } else {
      ...
      case 'with': {
        ...
        for (const cte of statement.ctes) {
          ...
          map.set(String(cte.name).toLowerCase(), sel);
        }
        ...
        ordered.forEach((rowIdx, pos) => {
          ...
          switch (w.fn) {
            ...
            case 'MAX': {
              ...
              v = this._aggValue(slice, w.fn, argNode);
              break;
            }
            default: v = null;
          }
  ...
  async executeSelect(statement) {
    ...
    if (!statement.groupBy && (statement.columns.some(c => c.aggregate && !c.window) || hasCustomAgg)) {
      const cols = statement.columns.map(c => scalarColumnName(c));
      const valueOf = (c) => {
    ...
    if (statement.groupBy) {
      ...
      return { ok: true, type: 'select', table: statement.from ? (statement.from.tables[0].table || null) : null, columns: cols, rows: mapped, raw: rowsForGroups };
    }

    if (statement.orderBy) {
      // ORDER BY 项可能是列名字符串，也可能是表达式节点
      const ordVal = (row, o) => (typeof o.column === 'string'
...
function splitStatements(sql) {
  ...
  while (i < sql.length) {
    ...
    if (inStr) {
      current += c;
      if (inStr === '`') {
        // 反引号标识符内部反斜杠不是转义符；两个连续反引号才表示一个字面反引号。
        if (c === '`' && sql[i + 1] === '`') { current += sql[i + 1]; i += 2; continue; }
        if (c === '`') inStr = null;
        i++;
        continue;
      }
    ...
    if (c === "'" || c === '"' || c === '`') { inStr = c; current += c; i++; continue; }
    if (c === '-' && sql[i + 1] === '-') {
