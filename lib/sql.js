...
class Parser {
  ...
  parseSelect() {
    ...
    return { type: 'select', columns, aggregate, distinct, from, where, groupBy, having, orderBy, limit, offset, union, intersect, except };
  }
  ...
  parseComparison() {
    ...
    if (t.type === 'keyword' && t.value === 'NOT') {
      ...
      // NOT 后必须紧跟比较关键字
      const n = this.peek();
      if (!['IN', 'BETWEEN', 'LIKE', 'REGEXP', 'RLIKE'].includes(n.value)) {
        throw new Error(`Expected IN, BETWEEN, LIKE, REGEXP or RLIKE after NOT, got '${n.value}'`);
      }
    }
    const t2 = this.peek();
...
class SQLExecutor {
  ...
  async _materialize(expr) {
    ...
    if (expr.type === 'exists' && expr.select) {
      ...
      expr._rowCount = await this._evalExists(expr, null);
      return;
    }
  ...
  /** 收集表达式树里的所有 exists 节点 */
  _collectExists(node, out = []) {
    if (!node || typeof node !== 'object') return out;
    if (Array.isArray(node)) { for (const n of node) this._collectExists(n, out); return out; }
    if (node.type === 'exists' && node.select) out.push(node);
    for (const k of Object.keys(node)) {
      if (k === 'select') continue;
      const v = node[k];
      if (v && typeof v === 'object') this._collectExists(v, out);
    }
    return out;
  }

  /**
   * 求值一个 EXISTS 子查询。outerRow 非空表示相关子查询：
   * 临时把外层行挂到 ctx.__outer，让 `e.dept` 这类外层引用能解析到。
   */
  async _evalExists(node, outerRow) {
  ...
  async executeSelect(statement) {
    ...
    let rows = all;
    if (statement.where) {
      // 相关子查询：EXISTS 必须逐行重新求值，否则外层引用拿不到当前行的值
      const existsNodes = this._collectExists(statement.where);
      if (existsNodes.length > 0) {
        const kept = [];
        for (const r of rows) {
          for (const node of existsNodes) node._rowCount = await this._evalExists(node, r);
