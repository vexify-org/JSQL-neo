...
class JSQL {
  constructor(opts = {}) {
    ...
    this._buffer = {};
    this._bufferSize = 0;
    this._opBuffer = { remove: {}, update: {}, find: {} };
    this._opTimer = null;
  ...
  _flushOps() {
    for (const [table, ids] of Object.entries(this._opBuffer.remove)) {
      if (ids.length === 0) continue;
      const idsArr = Array.from(ids);
      native.jsqlRemoveByIds(table, JSON.stringify(idsArr));
    }
    for (const [table, entries] of Object.entries(this._opBuffer.update)) {
      if (entries.length === 0) continue;
      native.jsqlUpdateByIds(table, JSON.stringify(entries));
    }
    for (const [table, entries] of Object.entries(this._opBuffer.find)) {
      if (entries.length === 0) continue;
      const idsArr = Array.from(entries.keys());
      const resultStr = native.jsqlFindByIds(table, JSON.stringify(idsArr));
      const results = JSON.parse(resultStr);
      if (results && Array.isArray(results)) {
        for (let i = 0; i < idsArr.length && i < results.length; i++) {
          const resolver = entries.get(idsArr[i]);
  ...
  async findById(table, id) {
    this._flushOpsNow();
    const r = JSON.parse(native.jsqlFindById(table, Number(id)));
    if (r && r.error) throw new Error(r.error);
    return r;
  }

  async findByIdBuffered(table, id) {
    if (!this._opBuffer.find[table]) this._opBuffer.find[table] = new Map();
    return new Promise((resolve) => {
      this._opBuffer.find[table].set(Number(id), resolve);
