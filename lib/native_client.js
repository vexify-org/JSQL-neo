...
class JSQL {
  constructor(opts = {}) {
    ...
    // native 引擎：钩子内再次调用引擎方法会触发 N-API 重入崩溃，需显式防护
    this._forbidReentry = true;
  ...
  async addColumn(table, name, def) {
    ...
    const r = safeParse(native.jsqlAddColumn(table, name, JSON.stringify(mapped)));
  ...
  async exists(table, filter = {}) {
    const rows = await this.find(table, filter, { limit: 1 });
