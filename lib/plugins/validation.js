...
function createValidation(opts = {}) {
  ...
  // install 时拿到插件上下文（schema 查询），供各钩子闭包使用
  let engineCtx = null;

  const validateRow = (table, row, partial) => {
    const schema = engineCtx && engineCtx.getTableSchema(table);
    if (!schema) return;
  ...
  const validateMany = (table, rows, partial) => {
    if (Array.isArray(rows)) rows.forEach(r => validateRow(table, r, partial));
    else if (rows && typeof rows === 'object') validateRow(table, rows, partial);
  };

  return definePlugin('validation', {
    install(engine, ctx) { engineCtx = ctx; },
    hooks: {
      beforeInsert(table, rows) { validateMany(table, rows, false); },
      beforeUpdate(table, second, third) {
        if (third !== undefined) { validateMany(table, third, true); return; }
        if (Array.isArray(second)) for (const p of second) if (Array.isArray(p)) validateMany(table, p[1], true);
      }
    }
  });
}
