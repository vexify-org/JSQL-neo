...
function buildPluginContext(engine, plugin) {
  ...
  const ctx = {
    ...
    /** 组合：加载另一个插件 */
    use(other) { return engine.use(other); },
    /** 表清单（同步） */
    tables() {
 ...
 * 引擎的 `use()` 只需 `return applyPlugin(this, plugin)`。
 * @returns {object} engine（便于链式调用）
 */
function applyPlugin(engine, plugin) {
  if (typeof plugin === 'function') plugin = { install: plugin };
  if (!plugin || typeof plugin !== 'object') {
    throw new Error('use: plugin must be an object or a function');
  }
