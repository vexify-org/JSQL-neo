...
class Query {
    ...
    get() {
        ...
        } else if (this._caseExpr) {
            // CASE already cloned
        } else {
            const copy = new Array(rows.length);
            for (let i = 0; i < rows.length; i++) copy[i] = { ...rows[i] };
            rows = copy;
        }
    ...
    exists() {
        const prev = this._limitCount;
        this._limitCount = 1;
        const rows = this.get();
        this._limitCount = prev;
    ...
    _applyHashJoin(localRows, foreignTable, join) {
        ...
        if (join.type === 'left') {
            ...
            for (const lr of localRows) {
                const key = lr[join.localField];
                if (key === undefined || key === null) continue;
                if (!localMap.has(key)) localMap.set(key, []);
                localMap.get(key).push(lr);
            }
            for (const fr of foreignTable._rows) {
                const matches = localMap.get(fr[join.foreignField]);
